            ...
            <div className="grid md:grid-cols-3 gap-4 md:gap-16">
                {organisersData.map((organiser, index) => (
                    <div className="flex flex-col gap-2" key={index}>
                        <div className='h-[340px] w-full rounded-xl bg-[#D9D9D9] overflow-hidden'>
                            <Image src={`/images/organisers${organiser.image}`} alt={organiser.name} width={300} height={340} className='h-full w-full object-cover rounded-xl transform transition-transform duration-300 hover:scale-110' />
                        </div>

                        <div className="text-textDark">
                            <h4 className=''> {organiser.name} </h4>
                            <p className='text-sm font-light'> {organiser.role} </p>
                        </div>
